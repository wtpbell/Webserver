...
TEST_CASE("GetRouteView(ip, port, hostName, targetPath) keeps listener scope", "[ServerRegistry]")
{
  std::string raw = "\n"
                    "http {\n"
                    "  server {\n"
...
TEST_CASE("listen invalid 2 - ipv6 normalization", "[ServerRegistry]")
{
  std::string raw = "\n"
                    "http {\n"
                    "\n"
...
TEST_CASE("listen invalid 3", "[ServerRegistry]")
{
  std::string raw = "http {\n"
                    "  server {\n"
  ...
  std::set<std::size_t> errorsIdx{47, 50, 53, 56, 59, 62, 65, 68, 71, 74, 77, 80, 85};
  for (size_t i = 0; i < lexer.GetSizeTokenList(); ++i)
  {
    if (errorsIdx.count(i) != 0)
    {
...
TEST_CASE("Servers with default listen values", "[ServerRegistry]")
{
  std::string raw =
      "\n"
      "http {\n"
...
TEST_CASE("server_name", "[ServerRegistry]")
{
  std::string raw = "\n"
                    "http {\n"
                    "\n"
...
TEST_CASE("one server, four locations", "[ServerRegistry]")
{
  std::string raw = "\n"
                    "http {\n"
